...
**Voice**
- Speech-to-text via Qwen3-ASR (local, MLX, 4-bit, hotword-biased)
...
**Wake-up Alarm + Morning Briefing**
- Daily alarm (6:30 AM default) via Telegram text, a spoken voice note, and Web Push
- Edit it in chat: "move my alarm to 6:45 on weekdays", "add my to-dos to the alarm", "have the alarm remind me to take vitamins", "turn off my alarm"; `/alarm` shows it, `/alarm test` fires it
- Only real data (weather, synced calendars, reminders, Dory) plus lines you add — nothing generated by default
- Contents: date, weather (high/low/rain for each place), today's events, due reminders, Dory signals
...
| STT | Qwen3-ASR-1.7B 4-bit (`mlx-audio`) |
...
# Wake-up alarm (initial default; change it later by asking Elwin)
ALARM_TIME="06:30"
GOOGLE_CALENDAR_URLS="https://calendar.google.com/calendar/ical/.../basic.ics"   # optional, read-only
APPLE_CALENDAR_URLS="webcal://p00-caldav.icloud.com/published/2/..."          # optional, read-only
WEATHER_LOCATIONS="Home=38.80,-77.05; Work=38.90,-77.04"   # Name=lat,lon; ...
...
├── calendar_sync.py    Read-only Google/iCal calendar sync (private ICS links)
├── weather.py          Open-Meteo forecast for WEATHER_LOCATIONS
├── alarm.py            Wake-up alarm: chat-editable schedule + delivery
...
- Weather from Open-Meteo for the alarm briefing (no account required)
